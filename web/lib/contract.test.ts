...
describe("describeError", () => {
  ...
  it("covers exactly codes 1..31, the contract's enum", () => {
    expect(describeError(new Error("Error(Contract, #31)"))).toMatch(/already owns/);
    expect(describeError(new Error("Error(Contract, #32)"))).toMatch(/code 32/);
  });
});
