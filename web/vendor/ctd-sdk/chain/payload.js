...
const scvBytes = (b) => xdr.ScVal.scvBytes(Buffer.from(b));
const pointVal = (p) => scvBytes(pointToBytes(p));
const fieldVal = (v) => scvBytes(toBytes32BE(v));
/** A contracttype struct: ScMap with symbol keys sorted ascending (byte order). */
export function scvStruct(fields) {
...
export function encodeRegisterData(w, proof) {
...
export function encodeWithdrawData(w, proof) {
    const p = w.payload;
    const payload = scvStruct({
...
export function encodeTransferData(w, proof) {
    const p = w.payload;
    const payload = scvStruct({
        c_spend_new: pointVal(p.cSpendNew),
        c_tx: pointVal(p.cTx),
