...
export function submitRegister(client, signer, account, auditorId, witness, proof) {
    return client.invoke(client.cfg.contracts.token, "register", [addr(account), xdr.ScVal.scvU32(auditorId), encodeRegisterData(witness, proof)], signer);
}
/** `deposit(from, to, amount)` — public → confidential, no proof. */
export function submitDeposit(client, signer, from, to, amount) {
    return client.invoke(client.cfg.contracts.token, "deposit", [addr(from), addr(to), i128(amount)], signer);
}
/** `merge(account)` — fold receiving balance into spendable, no proof. */
export function submitMerge(client, signer, account) {
...
export function submitWithdraw(client, signer, from, to, amount, witness, proof) {
    return client.invoke(client.cfg.contracts.token, "withdraw", [addr(from), addr(to), i128(amount), encodeWithdrawData(witness, proof)], signer);
}
/** `confidential_transfer(from, to, data)` — confidential → confidential. */
export function submitTransfer(client, signer, from, to, witness, proof) {
    return client.invoke(client.cfg.contracts.token, "confidential_transfer", [addr(from), addr(to), encodeTransferData(witness, proof)], signer);
}
