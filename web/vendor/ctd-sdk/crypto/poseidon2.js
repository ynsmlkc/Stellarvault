...
export function poseidonWithDomain(d, inputs) {
 ...
 */
export function spongeSqueeze2(d, sx, sigma) {
...
export const vkFromSk = (sk, addrF) => poseidonWithDomain(DOMAIN.VIEWING_KEY, [sk, addrF]);
/** `dvk = Poseidon2(DELEGATION_VIEWING_KEY, vk, op_i)`. */
export const dvkFromVkOp = (vk, opI) => poseidonWithDomain(DOMAIN.DELEGATION_VIEWING_KEY, [vk, opI]);
/** `r' = Poseidon2(SPEND_RANDOMNESS, vk, sigma)`. */
export const deriveSpendR = (vk, sigma) => poseidonWithDomain(DOMAIN.SPEND_RANDOMNESS, [vk, sigma]);
/** `r_a = Poseidon2(ALLOWANCE_RANDOMNESS, dvk, sigma_a)`. */
export const deriveAllowR = (dvk, sigmaA) => poseidonWithDomain(DOMAIN.ALLOWANCE_RANDOMNESS, [dvk, sigmaA]);
/** `r_tx = Poseidon2(TX_BLINDING, s, sigma)`. */
export const deriveTxBlind = (s, sigma) => poseidonWithDomain(DOMAIN.TX_BLINDING, [s, sigma]);
/**
 ...
 */
export const deriveEphemeralRE = (vk, sigma) => {
...
export const encryptAmount = (vTx, s, sigma) => frAdd(vTx, poseidonWithDomain(DOMAIN.TX_AMOUNT, [s, sigma]));
/** `b_tilde = v_new + Poseidon2(ENCRYPTED_BALANCE, vk, sigma)`. */
export const encryptBalance = (vNew, vk, sigma) => frAdd(vNew, poseidonWithDomain(DOMAIN.ENCRYPTED_BALANCE, [vk, sigma]));
/** `a_tilde = v_a + Poseidon2(ENCRYPTED_ALLOWANCE, dvk, sigma_a)`. */
export const encryptAllowance = (vA, dvk, sigmaA) => frAdd(vA, poseidonWithDomain(DOMAIN.ENCRYPTED_ALLOWANCE, [dvk, sigmaA]));
/** `escrowed_dvk = dvk + Poseidon2(ESCROWED_DELEGATION_VIEWING_KEY, s, op_i)`. */
export const encryptEscDvk = (dvk, s, opI) => frAdd(dvk, poseidonWithDomain(DOMAIN.ESCROWED_DELEGATION_VIEWING_KEY, [s, opI]));
/** `b_tilde_aud_s = v_new + Poseidon2(AUDITOR_SENDER, s_a_s_x, sigma)`. */
export const encryptAuditorSenderBalance = (vNew, sAsX, sigma) => frAdd(vNew, poseidonWithDomain(DOMAIN.AUDITOR_SENDER, [sAsX, sigma]));
/**
 ...
 */
export const encryptDisclosure = (vTx, sDiscX, nu) => frAdd(vTx, poseidonWithDomain(DOMAIN.DISCLOSURE, [sDiscX, nu]));
/**
 ...
 */
export const decryptWithDomain = (ciphertext, d, a, b) => frMod(ciphertext - poseidonWithDomain(d, [a, b]));
