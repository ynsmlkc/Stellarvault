...
export const G = Grumpkin.BASE;
/** Pedersen generator H (index 1). */
export const H = Grumpkin.fromAffine({ x: H_X, y: H_Y });
/** The point at infinity / identity element. */
export const IDENTITY = Grumpkin.ZERO;
 ...
 * multiplier).
 */
export function scalarMul(scalar, p) {
...
export function pointToBytes(p) {
...
export function pointFromBytes(b) {
    if (b.length !== 64)
        throw new Error(`expected 64 bytes, got ${b.length}`);
    let allZero = true;
    for (const byte of b) {
        if (byte !== 0) {
            allZero = false;
