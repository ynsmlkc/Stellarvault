...
import { DISCLOSE_SENDER_CIRCUIT_ID, DISCLOSURE_CIRCUIT_IDS, } from "./types.js";
...
export async function verifyDisclosure(params) {
    ...
    if (pinnedVk) {
        ...
        if (vk.length !== pinnedVk.length || !vk.every((b, i) => b === pinnedVk[i])) {
            throw new DisclosureVerifyError("vk-pinning", `circuit artifact does not match the pinned "${bundle.circuitId}" verification key`);
        }
        steps.push(`VK pinned: ${bundle.circuitId} artifact matches the shared ${vk.length}B verification key`);
    }
    ...
    if (!event) {
        throw new DisclosureVerifyError("resolve-event", `ref_E does not resolve to a token-contract event (ledger ${bundle.refE.ledger}, id ${bundle.refE.id}) — wrong reference${indexer ? "" : ", or outside the RPC's ~7-day retention window"}`);
    }
    if (event.type !== "transfer") {
        throw new DisclosureVerifyError("resolve-event", `disclosures cover Transfer events; ref_E resolved to "${event.type}"`);
    }
    steps.push(`Resolved ref_E on-chain: transfer ${event.from.slice(0, 6)}… → ${event.to.slice(0, 6)}… in tx ${event.txHash.slice(0, 10)}… (ledger ${event.ledger})`);
    ...
    if (!accountA) {
        throw new DisclosureVerifyError("resolve-account", `event ${isSender ? "sender" : "recipient"} ${disclosingAccount} has no confidential account record`);
    }
    steps.push(`Read PVK_A from the on-chain account at the event's "${isSender ? "from" : "to"}" address (${disclosingAccount.slice(0, 6)}…)`);
    let pvkB = null;
    if (isSender) {
        const accountB = await client.confidentialBalance(event.to);
        if (!accountB) {
            throw new DisclosureVerifyError("resolve-account", `event recipient ${event.to} has no confidential account record`);
        }
    ...
    const amount = decryptDisclosure(keys.rR, pointFromJson(bundle.rDisc), fromHex(bundle.vTildeDisc), fromHex(request.nu));
    if (amount >= 1n << 127n) {
